...
def get_star_keywords_prompt_template():
    # STAR 기법을 적용한 핵심 키워드 추출 프롬프트 (한글로 작성)
    template = """
    ...
    The extracted data should include important keywords that describe the user's involvement in Korean.

    - Projects: For each project, use the STAR method to extract keywords about the user's involvement in Korean:
    ...
    The output should be in a structured format that can be stored in MongoDB.
